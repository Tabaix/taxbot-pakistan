...
export default function ChatBot() {
  ...
  // Load history + check for saved API key on mount
  useEffect(() => {
    ...
    setHasCustomKey(!!localStorage.getItem('user_gemini_key'));
  }, []);

  // Save to localStorage when messages change
  useEffect(() => {
    if (messages.length > 0) localStorage.setItem('taxbot_chat', JSON.stringify(messages));
  ...
  const handleSend = async (text: string = input) => {
    ...
    // Read user's personal API key from localStorage (if they set one)
    const userApiKey = localStorage.getItem('user_gemini_key') || undefined;
    ...
    try {
      ...
      // Handle quota/key errors gracefully
      if (!res.ok) {
        const isQuota = res.status === 429;
        setMessages(prev => [...prev, {
          role: 'assistant',
          content: isQuota
            ? "⚠️ Daily free usage limit reached. Click the 🔑 key icon above to add your own free Gemini API key and continue chatting!"
            : `❌ Error: ${data.error || 'Something went wrong. Please try again.'}`,
          isError: true,
        }]);
        if (isQuota) setShowApiKeyModal(true);
        return;
      }
      ...
      if (assistantContent.includes(calculatePrefix)) {
        ...
        try {
          const aiInput = JSON.parse(parts[1]);
          ...
          console.error('Failed to parse tax JSON:', e);
        }
      }

      setMessages(prev => [...prev, { role: 'assistant', content: assistantContent, result: taxResult }]);
      ...
      <div className="max-w-3xl mx-auto h-[85vh] flex flex-col bg-white rounded-2xl shadow-xl border border-slate-100 overflow-hidden">
        ...
        <div className="bg-blue-600 text-white p-4 flex justify-between items-center shadow-md z-10">
          <div>
            ...
            <p className="text-blue-200 text-xs">Powered by Google Gemini AI</p>
          </div>
          <div className="flex items-center gap-2">
            {/* API Key button */}
            <button
              onClick={() => setShowApiKeyModal(true)}
              title="Set your own Gemini API key"
              className={`flex items-center gap-1.5 text-xs px-3 py-1.5 rounded-full border transition-colors ${
                hasCustomKey
                  ? 'bg-emerald-500 border-emerald-400 text-white'
                  : 'bg-blue-700 border-blue-500 text-blue-100 hover:bg-blue-800'
              }`}
            >
              🔑 {hasCustomKey ? 'My Key ✓' : 'Add Key'}
            </button>
            ...
            <div key={i} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
              <div className={`max-w-[85%] rounded-2xl p-4 ${
                msg.role === 'user'
                  ...
                  : msg.isError
                    ? 'bg-rose-50 border border-rose-200 text-rose-800 rounded-bl-none shadow-sm'
                    : 'bg-white border border-slate-200 text-slate-800 rounded-bl-none shadow-sm'
              }`}>
                <p className="whitespace-pre-wrap leading-relaxed">{msg.content}</p>

                {/* Inline "Add Key" button on quota error */}
